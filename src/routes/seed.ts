    ...
    try {
        ...
        for(let i=0; i<1000; i++){
            const seed_listenTo = await prisma.listenTo.create({
                data: {
                    ...
                    idArtist: all_artist[Math.floor(Math.random() * all_artist.length)].id,
                    idMusic: faker.number.int({min:1,max:100}),
                    idAlbum: faker.number.int({min:1,max:10}),
                    listenAt: faker.date.past(),
                }
            })
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            if (!existingLikeTo) {
              ...
              console.log(`Entry with idUser=${uniqueIdUser} and idMusic=${uniqueIdMusic} already exists.`);
            }
          }
